...
import { CalendarDays, CheckCircle2, FolderKanban, Plus } from "lucide-react"
...
export default async function PaginaProyectos() {
  const lista = await proyectos().consultarProyectos.listar({ limite: 100 })
    ...
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-bold text-text-primary sm:text-3xl">Proyectos</h1>
          <p className="mt-1 text-sm text-text-secondary">
            Fases, módulos, calendario e inversión: la propuesta que acepta cada cliente.
          </p>
        </div>
        <Link href="/panel/proyectos/nuevo" className={BOTON_NUEVO}>
      ...
      {lista.length === 0 ? (
        <div className="mt-8 flex flex-col items-center rounded-2xl border border-dashed border-border bg-surface p-12 text-center">
          ...
          <h2 className="mt-4 text-base font-semibold text-text-primary">Aún no hay proyectos</h2>
          <p className="mt-1 max-w-xs text-sm text-text-secondary">
            Créalo a partir de un levantamiento cuando ya tengas claro qué vas a construir.
          </p>
        </div>
      ) : (
        <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {lista.map((p) => {
            const avance = Math.round((p.seccionesCompletas / p.seccionesTotales) * 100)
            return (
              <Link
                key={p.id}
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <p className="text-xs font-semibold uppercase tracking-wider text-primary">{p.folio}</p>
                ...
                <h3 className="mt-2 truncate text-base font-semibold text-text-primary">{p.clienteNombre}</h3>
                <p className="mt-0.5 truncate text-sm text-text-secondary">
                  {p.proyectoNombre ?? "Proyecto sin nombre"}
                </p>
                <div className="mt-2 flex items-center justify-between gap-2 text-xs text-text-muted">
                  <span className="flex items-center gap-1.5">
                    <CalendarDays className="h-3.5 w-3.5" />
                    {formatearFecha(p.fechaPropuesta)}
                  </span>
                  {p.totalCentavos > 0 && (
                    <span className="font-semibold tabular-nums text-text-primary">
                      {formatearDinero(p.totalCentavos / 100, p.moneda)}
                    </span>
                  )}
                </div>
                <div className="mt-4 border-t border-border pt-4">
                  <div className="flex items-center justify-between text-xs text-text-muted">
                    ...
                    <span className="tabular-nums">
                      {p.seccionesCompletas}/{p.seccionesTotales}
                    </span>
                  </div>
