      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-2 text-[22px] font-bold text-text-primary sm:text-3xl">
            Tu problema + tu visión = una solución hecha a la medida de tu negocio
          </h2>
        </div>
        <div className="relative mt-12">
          <div className="absolute left-8 top-0 hidden h-full w-0.5 bg-gradient-to-b from-primary/40 to-primary/10 md:block" />
          <div className="space-y-8">
            {steps.map((step, i) => (
              <div key={step.title} className="relative flex items-start gap-6">
                <div className="relative z-10 flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-2xl bg-primary shadow-elevated">
                  <step.icon className="h-7 w-7 text-white" />
                </div>
                <div className="min-w-0 flex-1 rounded-xl border border-border bg-surface p-5 shadow-card transition-all hover:border-primary/20 hover:shadow-hover">
