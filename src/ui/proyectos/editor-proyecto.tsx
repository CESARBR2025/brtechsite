...
import {
  ...
  publicarProyecto,
  retirarAceptacion,
} from "@/src/modules/proyectos/infrastructure/acciones-proyectos"
import type { ResultadoAccion } from "@/src/modules/proyectos/infrastructure/tipos"
import type { ProyectoDetalleDTO } from "@/src/modules/proyectos/application/dtos"
import {
  type ClaveSeccion,
  type Contenido,
  completitud,
...
/**
 * Captura del proyecto (misma mecánica que el levantamiento). Guarda solo: cada cambio
...
export function EditorProyecto({
  ...
  const [generales, setGenerales] = useState<Generales>({
    ...
    fechaPropuesta: inicial.fechaPropuesta,
  })
  ...
  const [abiertas, setAbiertas] = useState<Set<ClaveSeccion>>(() => {
    ...
    return new Set([primera?.clave ?? "ficha"])
  })
...
function AccionesProyecto({
  ...
  return (
    <section className="mt-6 rounded-2xl border border-border bg-surface p-4 shadow-card sm:p-6">
      <div>
        <h2 className="text-sm font-semibold text-text-primary">Propuesta del cliente</h2>
        <p className="mt-0.5 text-xs text-text-muted">
          {publicado
            ? "Visible en su enlace. Los cambios que guardes se ven al recargar."
            : "Solo es visible en su enlace cuando está publicada."}
        </p>
      </div>

      {dto.aceptacion && (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-success-light px-4 py-3">
          ...
          {!archivado && (
            <button
              ...
              disabled={pendiente}
              onClick={() => {
                if (confirm("¿Retirar la aceptación? El cliente tendrá que volver a aceptar la propuesta.")) {
                  correr(() => retirarAceptacion(dto.id))
                }
              }}
              className="text-xs font-medium text-text-muted underline-offset-2 hover:text-red-500 hover:underline"
            >
              Retirar aceptación
            </button>
          )}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {publicado ? (
          <>
            ...
            <button
              type="button"
              disabled={pendiente}
              onClick={() => correr(() => despublicarProyecto(dto.id))}
          ...
          <button
            ...
            onClick={() => correr(() => publicarProyecto(dto.id))}
            className={`${BTN} bg-primary text-white shadow-lg shadow-primary/25 hover:bg-primary-hover`}
          >
        ...
        {!archivado && (
          <button
            ...
            disabled={pendiente}
            onClick={() => {
              if (confirm("¿Archivar este proyecto? Dejará de ser visible y ya no se podrá editar.")) {
                correr(() => archivarProyecto(dto.id))
              }
            }}
